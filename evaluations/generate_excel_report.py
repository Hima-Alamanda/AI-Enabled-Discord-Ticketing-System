...
def generate_professional_report():
    ...
    # Define the display name mapping with professional labels
    DISPLAY_NAME_MAP = {
    ...
    # --- STRATEGIC COLUMN REORDERING (Sheet 1) ---
    # Strategic Board Flow: Identity -> Issue -> QUALITY SCORES -> Semantic Score -> Infra (Token/Latency) -> Output
    leading_cols = ['Prompt_Strategy', 'Model', 'Case_ID', 'Case_Query']
    # Check what columns actually exist in the dataframe to avoid KeyErrors
    existing_leading = [c for c in leading_cols if c in df_raw.columns]
    quality_cols = [c for c in SCORE_COLS if c in df_raw.columns]
    accuracy_cols = [c for c in ACCURACY_COLS if c in df_raw.columns]
    infrastructure_cols = [c for c in TOKEN_COLS + [LATENCY_COL_DISPLAY] if c in df_raw.columns]
    trailing_cols = ['Model_Response', 'Expected_SOP_Content']
    existing_trailing = [c for c in trailing_cols if c in df_raw.columns]

    final_col_order = existing_leading + quality_cols + accuracy_cols + infrastructure_cols + existing_trailing
    # Add any columns we missed
    remaining = [c for c in df_raw.columns if c not in final_col_order]
    final_col_order = final_col_order + remaining
    
    # Reorder the dataframe
    df_raw = df_raw[final_col_order]
    ...
    # --- SHEET 3: TOKENS & COST ---
    # We want token usage data for cost analysis
    tokens_df = df_raw[existing_leading + TOKEN_COLS].copy()

    # --- SHEET 4: EXECUTIVE GLOSSARY & DEFINITIONS ---
    summary_data = [
        ["I. EXECUTIVE SUMMARY & REPORT OVERVIEW", ""],
        ["Context", "Comparing multi-model performance across disparate prompt engineering strategies."],
        ["", ""],
        ...
        ["", ""],
        ["IV. PERFORMANCE & RESOURCE LIMITS", ""],
        ...
        ["Latency (ms / sec)", "Response time taken by the system to generate an answer. Lower is better. Raw values are in milliseconds, while averages are shown in seconds for readability."],
        ["Tokens (Consumption)", "The values represent actual production consumption per ticket. Input includes user query + RAG context window; Output is the bot's generated response."],
        ["Gemini 2.5 Pro", "OCI Context Capacity: 128k+ Tokens | Configured Max Output: 6,000 Tokens"],
        ["Grok 4.20 Reasoning", "OCI Context Capacity: 128k+ Tokens | Configured Max Output: 600 Tokens"],
    ]
    ...
    with pd.ExcelWriter(OUTPUT_EXCEL, engine='openpyxl') as writer:
        ...
        for sheet_name in workbook.sheetnames:
            ...
            # Conditional Formatting (Only for Summary/Average Sheets)
